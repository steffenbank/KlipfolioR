...
#' Aquire groups from the Klipfolio API
#'
#' @param user user (Klipfolio account email)
#' @param password password (Klipfolio account password)
#'
#' @return List of groups obtainable by \code{user} with password: \code{password} with name and description
#' @export
#'
#' @examples
...
#' password <- c("changeme")
#'
#' klipfolio_groups(email,password)
klipfolio_groups <- function(user, password) {
  
  # ---------------------------------------------------------- #
  # obtain meta data
  # check api access
  # get number of calls needed
  api_meta <- httr::GET(paste("https://app.klipfolio.com/api/1.0/groups?limit=1"),httr::authenticate(user = user, password = password), encode = "XML")
  ...
  if(api_meta$status_code != 200) {
    ...
    for(i in 1:n_calls) {
      ...
      # query
      query <- paste("https://app.klipfolio.com/api/1.0/groups?limit=100&offset=",(i-1)*100, sep= "")
      
      if(i == 1) {
        
        list_raw_base <- httr::GET(query,httr::authenticate(user = user, password = password), encode = "XML")
        list_base <- httr::content(list_raw_base)$data$groups
        
      } else {
        
        list_raw_temp <- httr::GET(query,httr::authenticate(user = user, password = password), encode = "XML")
        list_temp <- httr::content(list_raw_temp)$data$groups
    ...
    names(df)[1] <- "group_id"
    names(df)[2] <- "group_name"
    names(df)[3] <- "group_description"
